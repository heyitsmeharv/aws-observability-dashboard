...
The sandbox demo enables Application Signals tracing by default. It does this with CommonJS ADOT auto-instrumentation inside the Node backend container plus a CloudWatch agent sidecar in the same ECS task.
...
For service maps, distributed traces, and correlated service-level views, the application must be onboarded to [AWS Application Signals on ECS](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Application-Signals-Enable-ECS.html). The module exposes trace drilldown links and can enable active tracing for canaries, but it still cannot generate service trace data if the workload emits none. The demo stack uses CommonJS ADOT auto-instrumentation in the backend container plus a CloudWatch agent sidecar. AWS still documents Node ESM support as limited.
